		...
		this.harness.configure({
			listeners: {
				...
				testfailedwithexception: function (event, test) {
				    var className = this.getClassNameFromUrl(test.url);
				    this.console.log(test.failedException);
...
Ext.define("JsTestRunner.Client.SignalR", {
	...
	initConnection: function () {
	    ...
	    var browserInfoParam = JSON.stringify(browserInfo);
	    this.connection.hub.qs = "clientType=JsTestRunner&browserInfo=" + browserInfoParam;
	    this.connection.hub.url = this.baseUrl;
	    this.connection.hub.reconnected(this.onReconected.bind(this));
		return this.connection.hub.start();
	},
    onReconected: function() {
        console.clear();
        console.warn("Js runner client reconnected...");
    },
	initHubs: function () {
